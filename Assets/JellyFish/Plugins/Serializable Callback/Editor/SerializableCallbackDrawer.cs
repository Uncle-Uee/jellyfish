...
public class SerializableCallbackDrawer : PropertyDrawer {
			...
			if (activeMethod != null && !dynamic) {
				...
				for (int i = 0; i < types.Length; i++) {
					...
					switch ((Arg.ArgType) argProp.FindPropertyRelative("argType").enumValueIndex) {
						...
						break;
						case Arg.ArgType.SubType:
							Type type = Type.GetType(argProp.FindPropertyRelative("subType").stringValue);
							argProp.FindPropertyRelative("objectValue").objectReferenceValue = EditorGUI.ObjectField(argRect,
							                                                                                         argProp
								                                                                                        .FindPropertyRelative("objectValue")
	...
	private Type[] GetArgTypes(SerializedProperty argProp) {
		...
		for (int i = 0; i < argProp.arraySize; i++) {
			Arg.ArgType argType = (Arg.ArgType) argProp.FindPropertyRelative("Array.data[" + i + "].argType").enumValueIndex;

			if(argType == Arg.ArgType.SubType)
			{
				types[i] = Type.GetType(argProp.FindPropertyRelative("Array.data[" + i + "].subType").stringValue);
			}
			else
			{
				types[i] = Arg.RealType(argType);
			}
		}
		return types;
	}
	...
	private void SetMethod(SerializedProperty property, UnityEngine.Object target, MethodInfo methodInfo, bool dynamic) {
		...
		argProp.FindPropertyRelative("Array.data[" + i + "].subType").stringValue = parameters[i].ParameterType.AssemblyQualifiedName;
		}
		property.FindPropertyRelative("dirty").boolValue = true;
