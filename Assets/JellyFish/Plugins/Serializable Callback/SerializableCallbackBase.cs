...
public abstract class SerializableCallbackBase : ISerializationCallbackReceiver {
	...
	public object[] args;
	public Type[] ArgTypes { get { return argTypes != null ? argTypes : argTypes = _args.Select(x => (x.argType == Arg.ArgType.SubType ? Type.GetType(x.subType) : Arg.RealType(x.argType))).ToArray(); } }
...
[System.Serializable]
public struct Arg {
	public enum ArgType { Unsupported, Bool, Int, Float, String, Object, SubType }
	...
	public static ArgType FromRealType(Type type) {
		...
		else if (type.IsSubclassOf(typeof(Object))) return ArgType.SubType;
